...
| `.maestro/06_add_record.yaml` | 이름 입력 → 저장 → 목록 끝까지 스크롤해 기록 확인 (`inputText`, `scrollUntilVisible`) |
| `.maestro/07_scroll_history.yaml` | 화면 밖의 마지막 예시 기록까지 스크롤해서 찾기 |
| `.maestro/08_async_wait.yaml` | 로딩 스피너 → 목록 표시 대기 → 뒤로가기 복귀 (`extendedWaitUntil`, `back`) |
